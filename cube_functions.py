...
class State:
    ...
    def swap_first_last_col(self, side1, side2):
        for i in range(len(side1)):
            side1[i][self.size - 1], side2[len(side1)-1-i][0] =  side2[len(side1)-1-i][0], side1[i][self.size - 1]
        return side1, side2
    ...
    #current constrain: cube rotates only in one direction: to overcome this we use multiple rotations
    def turn_front90(self, _reverse=False):
        # rotate the front side
        if(reverse):
            self.turn_front()
        self.__front__ = self.columns_to_rows(self.__front__)
        # Implement half turn for the sides
        top_row = self.__top__[0]
        bottom_row = self.__bottom__[0]
        left_col = []
        right_col = []
        for i in range(len(top_row)):
            right_col.append(self.__right__[len(top_row)-1-i][0]) 
            left_col.append(self.__left__[i][self.size - 1])
            self.__left__[i][self.size - 1], self.__right__[len(top_row)-1-i][0] = bottom_row[i],top_row[i]
        # get rest of rows of left_col
        self.__top__ = [left_col] + list(side for side in self.__top__[1:])
        # get rest of rows of right_col
        self.__bottom__ = [right_col] + list(side for side in self.__bottom__[1:])
        
    def turn_front(self, _90=False, reverse=False, T90=False):
        if _90 or T90==True:
            self.turn_front90(self, _reverse=reverse)
        else:
    ...
    def turn_top(self, T90=False):
        self.flip_cube(forward=True)
        self.turn_front(_90=T90)
        self.flip_cube()
    
    def turn_bottom(self, T90=False):
        self.flip_cube()
        self.turn_front(_90=T90)
        self.flip_cube(forward=True)
